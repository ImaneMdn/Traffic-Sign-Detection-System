...
const App = () => {
  ...
  const getPredication = async params => {
    return new Promise((resolve, reject) => {
      ...
      console.log('Request Payload:', bodyFormData);
      // eslint-disable-next-line prettier/prettier
      const url = 'https://us-central1-traffic-sign-detection-409319.cloudfunctions.net/predict';
      console.log('URL:', url);
      if (!url) {
        reject(new Error('Cannot load an empty url'));
      } else {
        return axios
  ...
  return (
    <View style={[backgroundStyle, styles.outer]}>
      ...
      />
      <Text style={styles.title}>{'Traffic Sign \nPrediction App'}</Text>
      ...
      {(result && label && (
        ...
        (image && <Text style={styles.emptyText}>{label}</Text>) || (
          <Text style={styles.emptyText}>
            Use below buttons to select a picture of a traffic sign.
          </Text>
        )}
